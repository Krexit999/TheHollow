...
export const ACHIEVEMENTS: AchievementDef[] = [
  ...
  A('firstKill', 7, 5, 'It Bit First', 'Fell one of the Deepwrought.', (s) => s.combat.stats.wins >= 1, 'strikePower', 1.05, '+5% strike power'),
  A('wardenLoam', 7, 6, 'The Root Gives Way', 'Fell the Tapmother.', (s) => s.combat.wardens.includes('loam'), 'strikePower', 1.05, '+5% strike power'),
  A('chords12', 9, 5, 'A Dozen Voices', 'Discover twelve Chords.', (s) => s.lattice.discovered.length >= 12, 'motifGain', 1.05, '+5% Motifs'),
  A('progressions3', 9, 6, 'Three Movements', 'Discover three Progressions.', (s) => s.lattice.discoveredProgressions.length >= 3, 'motifGain', 1.05, '+5% Motifs'),
  // Combat extras beyond the columns.
  A('kills25', 7, 7, 'Cull', 'Fell twenty-five Deepwrought.', (s) => s.combat.stats.wins >= 25, 'strikePower', 1.05, '+5% strike power'),
  A('wardenFerrite', 7, 8, 'The Star Goes Out', 'Fell the Loadstar.', (s) => s.combat.wardens.includes('ferrite'), 'strikePower', 1.08, '+8% strike power'),
  A('bestiary10', 6, 8, 'Field Notes', 'Encounter ten species.', (s) => s.combat.seen.length >= 10, 'dropRate', 1.05, '+5% drops'),
  A('perfect10', 6, 9, 'On the Beat', 'Land ten perfect strikes.', (s) => s.combat.stats.perfects >= 10, 'strikePower', 1.03, '+3% strike power'),
  A('gearFull', 8, 9, 'Dressed for the Dark', 'Wear gear in all four slots.', (s) => !!(s.forge.gear.offhand && s.forge.gear.lantern && s.forge.gear.harness && s.forge.gear.boots), 'dustYield', 1.05, '+5% chip yield'),
  ...
  A('firstAlloy', 8, 6, 'It Poured True', 'Discover an alloy.', (s) => s.crucible.discovered.length >= 1, 'dustYield', 1.03, '+3% chip yield'),
  A('alloys10', 8, 7, 'Shelf of Ingots', 'Discover ten alloys.', (s) => s.crucible.discovered.length >= 10, 'dustYield', 1.05, '+5% chip yield'),
  A('foundry1', 8, 8, 'First Fitting', 'Install a Foundry module.', (s) => s.foundry.installed.length >= 1, 'xpGain', 1.03, '+3% XP'),
  // Row 8 — Works
  ...
  A('scrip50', 0, 7, 'First Coin of the Road', 'Earn 50 Caravan Scrip.', (s) => (s.totals['scrip']?.gte(50)) ?? false, 'scripGain', 1.05, '+5% Scrip'),
  A('vessDeals5', 1, 7, "A Page in the Good Ledger", 'Five deals across Vess\'s counter.', (s) => s.guild.vess.deals >= 5, 'dustYield', 1.03, '+3% chip yield'),
  A('contracts5', 2, 7, 'Reliable Hands', 'Complete five contracts.', (s) => s.guild.contracts.completed >= 5, 'scripGain', 1.05, '+5% Scrip'),
  A('pages5', 3, 7, 'Letters from Below', "Find five of Sable's pages.", (s) => s.guild.sable.found.length >= 5, 'dropRate', 1.03, '+3% drops'),
  A('renown50', 4, 7, 'A Name at the Hearth', 'Earn 50 Renown.', (s) => (s.totals['renown']?.gte(50)) ?? false, 'xpGain', 1.03, '+3% XP'),
  A('firstHire', 5, 7, 'Signed On', 'Hire your first crew member.', (s) => Object.keys(s.guild.hirelings).length >= 1, 'drillSpeed', 1.03, '+3% drill speed'),
  A('chords15', 9, 7, "The Hermit's Nod", 'Discover fifteen Chords.', (s) => s.lattice.discovered.length >= 15, 'motifGain', 1.05, '+5% Motifs'),
  A('scrip500', 0, 8, 'A Working Purse', 'Earn 500 lifetime Scrip.', (s) => (s.totals['scrip']?.gte(500)) ?? false, 'scripGain', 1.05, '+5% Scrip'),
  A('marrowNod', 1, 8, "Marrow's Nod", 'Forge a tool at 80+ purity (his questline knows).', (s) => s.forge.tools.some((t) => t.purity >= 80), 'brickYield', 1.05, '+5% converter output'),
  A('contracts25', 2, 8, 'The Board Knows You', 'Complete twenty-five contracts.', (s) => s.guild.contracts.completed >= 25, 'scripGain', 1.08, '+8% Scrip'),
  A('loamBook', 3, 8, 'The Loam Book', "Recover every Loam page Sable left.", (s) => s.guild.sable.found.filter((id) => id <= 'p21').length >= 13, 'xpGain', 1.05, '+5% XP'),
  A('swornOne', 4, 8, 'Sworn Friend', 'Bring one of the thirty to Sworn.', (s) => Object.values(s.guild.npcs).some((n) => n.rep >= 250), 'scripGain', 1.05, '+5% Scrip'),
  A('loftThree', 5, 8, 'A Working Loft', 'Three hirelings drawing your colors.', (s) => Object.keys(s.guild.hirelings).length >= 3, 'offlineEffAdd', 0.01, '+1% offline efficiency'),
  A('progressions4', 9, 8, 'Four Movements', 'Discover four Progressions.', (s) => s.lattice.discoveredProgressions.length >= 4, 'motifGain', 1.05, '+5% Motifs'),
  // Lamphouse extras seeding column 10.
  A('ciphers5', 3, 9, 'Ciphers Broken', 'Pay five ciphered pages into the light.', (s) => s.guild.sable.translated.filter((id) => {
    const n = Number(id.slice(1));
    return n >= 29; // the ciphered run starts at page 29
  }).length >= 5, 'xpGain', 1.03, '+3% XP'),
  A('titled', 4, 9, 'A Name of Your Own', 'Equip a title from Sal\'s book.', (s) => s.guild.titles.equipped !== null, 'xpGain', 1.03, '+3% XP'),
  A('roadFive', 5, 9, 'The Road Turns', 'Send five caravan loads.', (s) => s.guild.caravan.trades >= 5, 'scripGain', 1.05, '+5% Scrip'),
  ...
  A('shapes4', 9, 9, 'Four Figures on the Frame', 'Weave four different shapes.', (s) => s.loom.discoveredShapes.length >= 4, 'motifGain', 1.05, '+5% Motifs'),
  ...
  A('brews10', 7, 10, 'A Working Cellar', 'Drink ten brews.', (s) => s.brewing.drunk >= 10, 'dropRate', 1.04, '+4% drops'),
  A('hybrids10', 8, 10, 'Ten New Names', 'Breed ten hybrids in the Greenhouse.', (s) => s.greenhouse.codex.filter((id) => id.startsWith('hy.')).length >= 10, 'dropRate', 1.05, '+5% drops'),
  A('myc12', 9, 10, 'The Long Thread', 'Grow the Mycelium to twelve nodes.', (s) => Object.keys(s.mycelium.nodes).length >= 12, 'motifGain', 1.05, '+5% Motifs'),
  // Verdance extras seeding column 12.
  A('wardenVerdance', 7, 11, 'Plenty, Refused', 'Fell Old Plenty.', (s) => s.combat.wardens.includes('verdance'), 'strikePower', 1.08, '+8% strike power'),
  A('feral9', 6, 11, 'A Wild Quarter', 'Let nine cells go feral at once.', (s) => s.growth.stage.filter((st) => st >= 4).length >= 9, 'regen', 1.04, '+4% regen'),
  A('codex40', 8, 11, 'The Green Book', 'Forty strains in the Greenhouse codex.', (s) => s.greenhouse.codex.length >= 40, 'dropRate', 1.05, '+5% drops'),
  ...
  A('lenses5', 9, 11, 'A Case of Lenses', 'Grind five Lenses at the Bench.', (s) => s.bench.solved.length >= 5, 'dropRate', 1.05, '+5% drops'),
  ...
  A('constellations3', 3, 12, 'Three Skies Closed', 'Complete three constellations.', (s) => s.observatory.constellations.length >= 3, 'xpGain', 1.05, '+5% XP'),
  A('pairs5', 4, 12, 'A Working Grammar', 'Discover five rune pairs.', (s) => s.runes.pairsSeen.length >= 5, 'strikePower', 1.05, '+5% strike power'),
  A('warrens6', 5, 12, 'Six Detours', 'Clear six Warrens.', (s) => Object.keys(s.warrens.cleared).length >= 6, 'dropRate', 1.05, '+5% drops'),
  A('observations25', 6, 12, 'The Long Ledger of Skies', 'Collect twenty-five observations.', (s) => s.observatory.completed >= 25, 'offlineEffAdd', 0.02, '+2% offline efficiency'),
  A('wardenGlassmere', 7, 12, 'It Blinked', 'Fell the Unblinking.', (s) => s.combat.wardens.includes('glassmere'), 'strikePower', 1.08, '+8% strike power'),
  A('uniques10', 8, 12, 'Ten Once-Onlys', 'Claim ten Warren uniques.', (s) => s.warrens.uniques.length >= 10, 'dropRate', 1.05, '+5% drops'),
  A('bench15', 9, 12, 'Fifteen Solved Lights', 'Solve fifteen Bench studies.', (s) => s.bench.solved.length >= 15, 'motifGain', 1.05, '+5% Motifs'),
  ...
  A('wardenCinder', 3, 14, 'Embers, After All', 'Fell the Smolder.', (s) => s.combat.wardens.includes('cinder'), 'strikePower', 1.08, '+8% strike power'),
  A('tool15', 4, 14, 'The Fifteenth', 'Forge the Cinder Maul. The ladder ends.', (s) => s.forge.tools.some((t) => t.tier >= 15), 'strikePower', 1.1, '+10% strike power'),
  ...
  A('tape1', 6, 15, 'The First Sentence', 'Record and run a tape in the Echo Chamber.', (s) => s.chamber.loops >= 1, 'dustYield', 1.04, '+4% chip yield'),
  ...
  A('pairs20', 9, 15, 'A Fluent Grammar', 'Know twenty rune pairs.', (s) => s.runes.pairsSeen.length >= 20, 'strikePower', 1.06, '+6% strike power'),
  ...
  A('wardenHollow', 2, 16, 'Seen, Once', 'Fell the Unattended.', (s) => s.combat.wardens.includes('hollow'), 'strikePower', 1.08, '+8% strike power'),
  ...
  A('bestProgram', 4, 16, 'The Shortest Sentence', 'Set a Chamber best above 10 yield per step.', (s) => s.chamber.bestEfficiency >= 10, 'dustYield', 1.05, '+5% chip yield'),
  A('resonance1k', 5, 16, 'A Thousand Returns', 'Earn 1K Resonance.', (s) => (s.totals['resonance']?.gte(1000)) ?? false, 'motifGain', 1.05, '+5% Motifs'),
  A('lens2', 6, 16, 'Both Eyes Open', 'Mount two Lenses at once.', (s) => s.bench.equippedLens !== null && s.bench.equippedLens2 !== null, 'dropRate', 1.05, '+5% drops'),
  A('pages40', 7, 16, 'Most of Her', 'Hold forty of Sable\'s pages.', (s) => s.guild.sable.found.length >= 36, 'xpGain', 1.06, '+6% XP'),
  A('quests28', 8, 16, 'The Hall\'s Own', 'Advance 28 questlines at least one step.', (s) => Object.values(s.guild.npcs).filter((n) => n.questStep > 0).length >= 28, 'scripGain', 1.1, '+10% Scrip'),
  A('heirloom1', 9, 16, 'It Outlived Its World', 'Own an heirloom tool.', (s) => s.forge.tools.some((t) => t.heirloom === true), 'strikePower', 1.05, '+5% strike power'),
  A('breach6', 0, 17, 'The Last Fall', 'Breach into Aleph.', (s) => s.shell.breachCount >= 6, 'dustYield', 1.1, '+10% chip yield'),
  A('wardenAleph', 1, 17, 'The Interrupted', 'Fell the Author.', (s) => s.combat.wardens.includes('aleph'), 'strikePower', 1.1, '+10% strike power'),
  ...
  A('page01', 3, 17, 'Marginalia', 'Read the first page, last.', (s) => s.guild.sable.read.includes('p01'), 'xpGain', 1.08, '+8% XP'),
  ...
  A('titles50', 0, 18, 'Half the Book of Names', 'Earn fifty titles.', (s) => s.guild.titles.earned.length >= 50, 'scripGain', 1.1, '+10% Scrip'),
  A('warrens20', 1, 18, 'Every Detour Taken', 'Clear all twenty-eight Warrens... or twenty, at least.', (s) => Object.keys(s.warrens.cleared).length >= 20, 'dropRate', 1.08, '+8% drops'),
  A('constellations8', 2, 18, 'All Eight Skies', 'Complete every constellation.', (s) => s.observatory.constellations.length >= 8, 'xpGain', 1.06, '+6% XP'),
  A('lenses50', 3, 18, 'The Full Case', 'Grind all fifty authored Lenses.', (s) => s.bench.solved.filter((id) => id.startsWith('auth')).length >= 50, 'dropRate', 1.06, '+6% drops'),
  A('alloys30', 4, 18, 'Half the Book of Metal', 'Discover thirty alloys.', (s) => s.crucible.discovered.length >= 30, 'kilnRate', 1.06, '+6% converter intake'),
  A('bestiary80', 5, 18, 'The Book of Teeth, Nearly Full', 'See eighty species.', (s) => s.combat.seen.length >= 80, 'strikePower', 1.06, '+6% strike power'),
  A('chords30', 6, 18, 'A Long Chorus', 'Discover thirty chords.', (s) => s.lattice.discovered.length >= 30, 'motifGain', 1.06, '+6% Motifs'),
  A('sustain20', 7, 18, 'The Twenty-Minute Fire', 'Hold the Array band for twenty minutes.', (s) => s.ember.bestSustainSec >= 1200, 'kilnRate', 1.08, '+8% converter intake'),
  ...
  // Row 9 — The Lattice
  A('firstMotif', 9, 0, 'Set the First Stone', 'Place a Motif on the Lattice.', (s) => s.lattice.placeSeq >= 1, 'motifGain', 1.02, '+2% Motifs'),
  A('firstChord', 9, 1, 'Older Than Music', 'Discover a Chord.', (s) => s.lattice.discovered.length >= 1, 'dustYield', 1.02, '+2% Dust'),
  A('chords5', 9, 2, 'Chorus', 'Discover five Chords.', (s) => s.lattice.discovered.length >= 5, 'motifGain', 1.05, '+5% Motifs'),
  A('ring2', 9, 3, 'Widening Circles', 'Expand the Lattice to its second ring.', (s) => s.lattice.rings >= 2, 'motifGain', 1.03, '+3% Motifs'),
  A('firstProgression', 9, 4, 'The Order of Things', 'Discover a Progression.', (s) => s.lattice.discoveredProgressions.length >= 1, 'xpGain', 1.05, '+5% XP'),
];
...
export const COL_BONUSES: Record<number, { bucket: Bucket; value: number; label: string }> = {
  ...
  5: { bucket: 'strikePower', value: 1.15, label: 'Column VI complete (+15% strike power)' },
  6: { bucket: 'xpGain', value: 1.15, label: 'Column VII complete (+15% XP)' },
  7: { bucket: 'scripGain', value: 1.15, label: 'Column VIII: The Lamphouse (+15% Scrip)' },
  ...
  14: { bucket: 'strikePower', value: 1.2, label: 'Column XV: Restraint (+20% strike power)' },
