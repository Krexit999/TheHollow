...
export const SYSTEM_COPY: Partial<Record<TabId, SystemCopy>> = {
  ...
  relics: {
    title: 'Relics',
    purpose:
      'The things you carried up. What a relic can carry is decided by where you found it, so you can hunt the shape you want instead of praying at a slot machine — and fusing one into another keeps the better of every line, so a duplicate is always progress and never waste. A relic can only ever leave your hands two ways: fed into a fusion, or given to a Museum case. LOCK one and neither can touch it.',
    status: (s) => {
    ...
    next: (s) => {
      ...
      if (s.relics.equipped.length === 0) return 'Everything you own is in the bag. A relic on the shelf does nothing — wear one.';
      ...
      return `${s.relics.equipped.length} of six worn. The empty slots are pure loss.`;
    },
  },
  museum: {
    title: 'The Museum',
