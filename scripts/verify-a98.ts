/**
 * A.98 DRIVEN IN THE REAL GAME — one live defect, one correction, the bank.
 *
 ...
 *
 *   npx tsx scripts/verify-a98.ts [port] [outDir]
 */
...
const OUT = process.argv[3] ?? 'sim-out/shots-a98';
const W = 380, H = 1700;
...
async function main(): Promise<void> {
  ...
  await page.locator('[data-testid="seats-panel"]').first()
    .screenshot({ path: `${OUT}/a98-seats-outlines.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="seats-panel"]').first()
    .screenshot({ path: `${OUT}/a98-seats-named.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="axiom-panel"]').first()
    .screenshot({ path: `${OUT}/a98-axioms.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="seats-panel"]').first()
    .screenshot({ path: `${OUT}/a98-seats-full.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="specify-panel"]').first()
    .screenshot({ path: `${OUT}/a98-specify.png` }).catch(() => {});
  ...
  for (const [t, id] of [
    ...
    await el.screenshot({ path: `${OUT}/a98-${id}.png` }).catch(() => {});
  }
