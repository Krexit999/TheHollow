/**
 * A.99 DRIVEN IN THE REAL GAME — one ruled trim, then three open systems.
 *
 ...
 *
 *   npx tsx scripts/verify-a99.ts [port] [outDir]
 */
...
const OUT = process.argv[3] ?? 'sim-out/shots-a99';
const W = 380, H = 1700;
...
async function main(): Promise<void> {
  ...
  await page.locator('[data-testid="seats-panel"]').first()
    .screenshot({ path: `${OUT}/a99-seats-outlines.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="seats-panel"]').first()
    .screenshot({ path: `${OUT}/a99-seats-named.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="axiom-panel"]').first()
    .screenshot({ path: `${OUT}/a99-axioms.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="seats-panel"]').first()
    .screenshot({ path: `${OUT}/a99-seats-full.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="specify-panel"]').first()
    .screenshot({ path: `${OUT}/a99-specify.png` }).catch(() => {});
  ...
  for (const [t, id] of [
    ...
    await el.screenshot({ path: `${OUT}/a99-${id}.png` }).catch(() => {});
  }
  ...
  await page.locator('[data-testid="crews-panel"]').first()
    .screenshot({ path: `${OUT}/a99-crews.png` }).catch(() => {});
