/**
 * A.101 DRIVEN IN THE REAL GAME — the corridor, the fifth defect, the gloves.
 *
 ...
 *   V  A.101: three floods in ONE corridor, from authored content
 *   W  A.101: §31's fifth defect firing, off and on
 *   X  A.101: gloves marking clean rock, and changing nothing else
 *
 ...
 *
 *   npx tsx scripts/verify-a101.ts [port] [outDir]
 */
...
const OUT = process.argv[3] ?? 'sim-out/shots-a101';
const W = 380, H = 1700;
...
async function main(): Promise<void> {
  ...
  await page.locator('[data-testid="seats-panel"]').first()
    .screenshot({ path: `${OUT}/a101-seats-outlines.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="seats-panel"]').first()
    .screenshot({ path: `${OUT}/a101-seats-named.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="axiom-panel"]').first()
    .screenshot({ path: `${OUT}/a101-axioms.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="seats-panel"]').first()
    .screenshot({ path: `${OUT}/a101-seats-full.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="specify-panel"]').first()
    .screenshot({ path: `${OUT}/a101-specify.png` }).catch(() => {});
  ...
  for (const [t, id] of [
    ...
    await el.screenshot({ path: `${OUT}/a101-${id}.png` }).catch(() => {});
  }
  ...
  await page.locator('[data-testid="crews-panel"]').first()
    .screenshot({ path: `${OUT}/a101-crews.png` }).catch(() => {});
  ...
  await page.locator('[data-testid="reserved-strip"]').first()
    .screenshot({ path: `${OUT}/a101-reserved.png` }).catch(() => {});
  ...
  // ═══ V — THREE FLOODS IN ONE CORRIDOR, FROM AUTHORED CONTENT ═════════════
  console.log('\n== V — the Heatworks, built with the real verb ==================');
  const corridor = await page.evaluate(async () => {
    const roll = await import(/* @vite-ignore */ '/src/engine/systems/roll' + '.ts');
    const flood = await import(/* @vite-ignore */ '/src/engine/systems/flood' + '.ts');
    const cond = await import(/* @vite-ignore */ '/src/engine/systems/condition' + '.ts');
    const w = window as unknown as Record<string, { getState: () => never }>;
    const s = w['__engine']!.getState() as unknown as {
      shell: { current: string }; depth: number; depthRecords: Record<string, number>;
      pressure: { heat: number }; spec?: unknown;
      plant: { tiers: Record<string, number>; condition?: Record<string, unknown> };
      roll: { flooded: string[]; floodgate?: boolean; rolled: Record<string, unknown> };
      casting: { rack: unknown[]; nextId: number };
      currencies: Record<string, { mul: (n: number) => { add: (n: number) => unknown } }>;
    };
    s.spec = undefined;                                  // no poured world here
    s.shell.current = 'cinder';
    s.plant.tiers['crusher'] = 1;
    s.plant.condition = {};
    s.roll.flooded = [];
    ...
    // THE CONTENT, read off the Roll — never a list in this file.
    const floods = (roll.shellRoll(s) as Array<{ id: string; depth: number; name: string }>)
      .filter((d) => (d as unknown as { type: string }).type === 'flood')
      .sort((a, b) => a.depth - b.depth);

    // The gate is a place, and a flood wants your depth record.
    const gate = flood.floodgateStation(s) as { depth: number };
    s.depthRecords['cinder'] = gate.depth;
    s.roll.floodgate = true;
    for (let i = 0; i < 60; i++) {
      s.casting.rack.push({ id: s.casting.nextId++, type: 'core', materialId: 'marl', purity: 50 });
    }
    s.currencies['ember'] = s.currencies['dust']!.mul(0).add(1e18) as never;

    const noop = { emit: Function.prototype, dirty: Function.prototype };
    const works = floods.slice(0, 3);
    s.depth = works[1]!.depth;
    const before = (cond.leakingStations(s) as string[]).length;
    const okAll: boolean[] = [];
    for (const d of works) {
      // The rng is INLINE and never bound to a name — see the header's note on
      // `__name`. A `const rng = () => 0.5` here cost this run once already.
      okAll.push((flood.floodStation(s, noop, d.id, () => 0.5) as { ok: boolean }).ok);
    }
    const felt = (cond.leakingStations(s) as string[]).length;
    const hot = cond.leakedHeat(s) as number;
    s.depth = floods[3]!.depth;                          // up at the lone Choke
    const away = (cond.leakingStations(s) as string[]).length;
    return {
      names: floods.map((d) => `${d.name}@${d.depth}`),
      before, okAll, felt, hot, away,
      spans: works.map((d) => Math.abs(d.depth - works[1]!.depth)),
    };
  });
  console.log(`        ${corridor.names.join('  ')}`);
  check(corridor.okAll, [true, true, true], [false, false, false],
    'V — three floods accepted, through `floodStation` and both its real gates');
  check([corridor.before, corridor.felt], [0, 3], [0, 0],
    'V — standing at the Bank, all THREE leak into the plant');
  check(corridor.hot > 0, true, false, 'V — ...and the plant reads hotter than the shaft');
  check(corridor.away, 0, 3,
    'V — walking up to the Choke leaves it behind: a place, not a buff');

  // ═══ W — §31's FIFTH DEFECT ══════════════════════════════════════════════
  console.log('\n== W — nothing down there answers ==============================');
  const defect = await page.evaluate(async () => {
    const crews = await import(/* @vite-ignore */ '/src/engine/systems/crews' + '.ts');
    const roll = await import(/* @vite-ignore */ '/src/engine/systems/roll' + '.ts');
    const spec = await import(/* @vite-ignore */ '/src/engine/systems/specify' + '.ts');
    const w = window as unknown as Record<string, { getState: () => never }>;
    const s = w['__engine']!.getState() as unknown as {
      shell: { current: string }; spec?: unknown; roll: { rolled: Record<string, unknown> };
    };
    s.shell.current = 'loam';
    const stops = roll.shellRoll(s) as Array<{ id: string; type: string; seams?: string[] }>;
    const seamed = stops.find((d) => d.type === 'seam' && (d.seams ?? []).length > 0)!;
    s.roll.rolled[seamed.id] = { seam: seamed.seams![0]!, feature: 'plain', hazard: 0 };
    const crew = { id: 1, name: 'Crew I', driftId: seamed.id, tier: 9, reads: ['seam'],
      gear: {}, atIndex: 0, timer: 0, recalled: false, findings: [], checked: [] };

    s.spec = undefined;
    const ordinary = crews.findingAt(s, crew, seamed);
    s.spec = { bands: ['cinder', 'loam', 'hollow'], defect: 'blindcrews', live: true, poured: 1, learned: [] };
    const specified = crews.findingAt(s, crew, seamed) as { kind: string; line: string } | null;
    const defects = (spec.DEFECTS as readonly string[]).slice();
    // ...and it still WALKS: the drift has stops either way.
    const stillWalks = (crews.driftStations(s, seamed.id) as unknown[]).length >= 0;
    s.spec = undefined;
    return {
      ordinary: ordinary === null, kind: specified?.kind ?? null,
      line: specified?.line ?? '', defects, stillWalks,
    };
  });
  console.log(`        defects: ${defect.defects.join(' · ')}`);
  console.log(`        says: ${defect.line}`);
  check(defect.defects.length, 5, 4, 'W — five defects, not four');
  check([defect.ordinary, defect.kind], [true, 'call'], [false, null],
    'W — the same crew with the same read: calls it normally, cannot in a specified world');
  check(defect.stillWalks, true, false,
    'W — ...and it still WALKS — the defect costs a capability, not the system');

  // ═══ X — THE GLOVES ══════════════════════════════════════════════════════
  console.log('\n== X — a swing that finds nothing still leaves a mark ===========');
  const gloves = await page.evaluate(async () => {
    const crews = await import(/* @vite-ignore */ '/src/engine/systems/crews' + '.ts');
    const roll = await import(/* @vite-ignore */ '/src/engine/systems/roll' + '.ts');
    const w = window as unknown as Record<string, { getState: () => never }>;
    const noop = { emit: Function.prototype, dirty: Function.prototype };
    const out: Array<{ cleared: number; findings: number }> = [];
    let paceSame = false;
    for (const worn of [{}, { gloves: 'chalkgloves' }]) {
      const s = w['__engine']!.getState() as unknown as {
        shell: { current: string }; spec?: unknown; crews?: unknown;
        roll: { shored: string[]; flooded: string[] };
      };
      s.spec = undefined;
      s.shell.current = 'loam';
      s.crews = { crews: [], nextId: 1, resolved: 0 };
      s.roll.shored = [];
      const stops = [...(roll.shellRoll(s) as Array<{ id: string; depth: number }>)]
        .sort((a, b) => a.depth - b.depth);
      s.roll.shored.push(stops[4]!.id);
      crews.dispatchCrew(s, noop, stops[4]!.id);
      const crew = (crews.ensureCrews(s) as { crews: Array<Record<string, unknown>> }).crews[0]!;
      crew['gear'] = worn;
      paceSame = (crews.crewPace(crew) as number) === (crews.STATION_SEC as number);
      for (let i = 0; i < (crews.STATION_SEC as number) * 25; i++) crews.tickCrews(s, noop, 1);
      const row = (crews.crewsRead(s) as { rows: Array<{ cleared: number; findings: unknown[] }> }).rows[0]!;
      out.push({ cleared: row.cleared, findings: row.findings.length });
    }
    return { bare: out[0]!, gloved: out[1]!, paceSame };
  });
  console.log(`        bare:   ${gloves.bare.cleared} clean · ${gloves.bare.findings} findings`);
  console.log(`        gloved: ${gloves.gloved.cleared} clean · ${gloves.gloved.findings} findings`);
  check([gloves.bare.cleared === 0, gloves.gloved.cleared > 0], [true, true], [true, false],
    'X — only a gloved crew records the rock it found nothing at');
  check(gloves.gloved.findings, gloves.bare.findings, gloves.bare.findings + 1,
    'X — ...and the findings are identical: information, never yield');
  check(gloves.paceSame, true, false, 'X — gloves do not change the pace either');
